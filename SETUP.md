...
```bash
direnv exec . uv run pytest tests/py_lib_testkit -m hermetic -q
direnv exec . uv run pytest tests/py_lib_testkit/integration/test_config_lifecycle.py -q --no-cov
```
...
and reselect `py-lib-testkit (.venv)`.
